...
let todos$ = new Rx.Subject().subscribe(() => {
	...
	for(let key in todos) {
		...
		if(filterBool === undefined) {
			if(todos[key].completed)
				todosList.insertAdjacentHTML( 'beforeend', `<li class="todos-item todos-item__completed" id="task-${key}">` + template + `</li>`);
			else	
				todosList.insertAdjacentHTML( 'beforeend', `<li class="todos-item" id="task-${key}">` + template + `</li>`);
		} else if (filterBool) {
	...
	for(let i = 0; i < done.length; i++) {
		Rx.Observable.fromEvent(done[i], 'click')
			.subscribe((e) => {
	...
	for(let i = 0; i < deleteItems.length; i++) {
		Rx.Observable.fromEvent(deleteItems[i], 'click')
			.subscribe((e) => {
				let id = e.target.id.split('-')[1];
...
Rx.Observable.fromEvent(allTasks, 'click')
	.subscribe(() => { 
...
Rx.Observable.fromEvent(activeTasks, 'click')
	.subscribe(() => { 
		filterBool = false;
...
Rx.Observable.fromEvent(completedTasks, 'click')
	.subscribe(() => { 
		filterBool = true;
